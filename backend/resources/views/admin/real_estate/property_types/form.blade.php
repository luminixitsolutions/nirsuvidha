...
@extends('admin.layouts.shell', ['title' => ($isEdit ? 'Edit property type' : 'Add property type') . ' — NRI Suvidha Admin'])

@include('admin.real_estate._styles')

@section('content')
<div class="re-card-form re-form">
    <h1>{{ $isEdit ? 'Edit property type' : 'Add property type' }}</h1>

    <form method="post" action="{{ $isEdit ? route('admin.property_types.update', $m) : route('admin.property_types.store') }}">
        @csrf
        @if($isEdit)
            @method('PUT')
        @endif
        ...
        <div style="display:flex;gap:0.75rem;margin-top:1.25rem;flex-wrap:wrap;">
            ...
            <a class="re-btn-ghost" href="{{ route('admin.property_types.index') }}">Back to list</a>
        </div>
    </form>
</div>
@endsection
