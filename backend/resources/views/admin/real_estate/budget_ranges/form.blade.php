...
@extends('admin.layouts.shell', ['title' => ($isEdit ? 'Edit budget range' : 'Add budget range') . ' — NRI Suvidha Admin'])

@include('admin.real_estate._styles')

@section('content')
<div class="re-card-form re-form">
    <h1>{{ $isEdit ? 'Edit budget range' : 'Add budget range' }}</h1>

    <form method="post" action="{{ $isEdit ? route('admin.budget_ranges.update', $m) : route('admin.budget_ranges.store') }}">
        @csrf
        @if($isEdit)
            @method('PUT')
        @endif
        ...
        <div class="re-field">
            ...
            <input id="label" name="label" type="text" value="{{ old('label', $m->label) }}" required placeholder="e.g. Under ₹50 Lakhs" />
        </div>
        <div class="re-field">
        ...
        <div style="display:flex;gap:0.75rem;margin-top:1.25rem;flex-wrap:wrap;">
            ...
            <a class="re-btn-ghost" href="{{ route('admin.budget_ranges.index') }}">Back to list</a>
        </div>
    </form>
</div>
@endsection
